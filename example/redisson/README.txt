...
如果使用setnx做分布式锁的时候, 会有锁续期的问题, 也就是说如果锁过期时间10s,
而业务处理花了30s, 而redisson就可以处理恰好处理这类锁续期的问题
而这类自动续期的机制就是所谓的看门狗机制,
总的来说就是默认锁过期时间是30s, 而自动续期机制在源码当中就是开启了定时任务,
定时间隔是看门狗时间的三分之一, 也就是10s, 所以就是在业务没有处理完的情况下锁默认每隔10s续期到30s.
...
分布式锁一定是在事务开始之前, 防止同时开启多个事务, 造成脏数据.
